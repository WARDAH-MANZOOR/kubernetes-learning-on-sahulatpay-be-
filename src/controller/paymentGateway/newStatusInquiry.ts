...
const statusInquiry = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if ((transaction?.providerDetails as JsonObject)?.name as string == "JazzCash") {
            result = await jazzCashService.statusInquiry(req.query, merchantId as string)
            res.status(200).json(ApiResponse.success(result))
            return;
        }
        else if ((transaction?.providerDetails as JsonObject)?.name as string == "Easypaisa") {
            const channel = (await transactionService.getMerchantChannel(merchantId as string))?.easypaisaPaymentMethod;
            const method = (await transactionService.getMerchantInquiryMethod(merchantId as string))?.easypaisaInquiryMethod;
            const obj = {...req.query, orderId: req.query.transactionId}
            if (method == "WALLET") {
                if (channel == "DIRECT") {
                    result = await easyPaisaService.easypaisainquiry(obj, merchantId as string);
                }
                else {
