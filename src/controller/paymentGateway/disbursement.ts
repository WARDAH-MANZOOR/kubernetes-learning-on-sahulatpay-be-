...
const getAllMerchantsWalletBalancesController = async ( 
  ...
  try {
    const timerLabel = `getAllMerchantsWalletBalances-${Date.now()}-${Math.random()}`;
    console.time(timerLabel);

    const rawStartDate = (req.query.startDate || req.query.start) as string | undefined;
    const rawEndDate = (req.query.endDate || req.query.end) as string | undefined;

    const startDate = rawStartDate?.trim();
    const endDate = rawEndDate?.trim();

    if ((startDate && !endDate) || (!startDate && endDate)) {
      throw new CustomError("Both start and end must be provided together", 400);
    }
    ...
    if ((start && Number.isNaN(start.getTime())) || (end && Number.isNaN(end.getTime()))) {
      throw new CustomError("Invalid date format for start or end", 400);
    }

    if (start && end && start > end) {
      throw new CustomError("start cannot be greater than end", 400);
    }
    ...
    const todayIncomeMap = new Map(
      allTodayIncome.map((b) => [b.merchant_id, b._sum.original_amount?.toNumber() || 0])
    );
    ...
    const balances = merchants.map((merchant) => ({
      ...
      companyName: merchant.company_name,
      walletBalance: walletMap.get(merchant.merchant_id) || 0,
      disbursementBalance: merchant.balanceToDisburse?.toNumber() || 0,
...
const getWalletBalanceControllerWithKey = async (
  ...
  try {
    const balance: any = await getWalletBalanceWithKey(merchantId as string);
    res.status(200).json(ApiResponse.success({ ...balance }));
  } catch (error) {
    next(error); // Pass the error to the error handling middleware
  }
...
const getDisbursementBalanceControllerWithKey = async (
  ...
  try {
    const balance: any = await getDisbursementBalanceWithKey(merchantId as string);
    res.status(200).json(ApiResponse.success({ ...balance }));
  } catch (error) {
    next(error); // Pass the error to the error handling middleware
  }
