...
const initiateSwichController = async (
  ...
  try {
    ...
    const result = await swichService.initiateSwich(req.body, merchantId as string);
...
const getSwichMerchant = async (
  ...
  try {
    ...
    const merchant = await swichService.getMerchant(merchantId as string);
...
const deleteSwichMerchant = async (
  ...
  try {
    ...
    const merchantId = req.params.merchantId;
    const deletedMerchant = await swichService.deleteMerchant(merchantId as string);
    if (!deletedMerchant) {
       res.status(404).json(ApiResponse.error("Merchant not found"));
       return
