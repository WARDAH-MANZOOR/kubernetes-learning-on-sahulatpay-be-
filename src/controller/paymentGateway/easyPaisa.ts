...
const initiateEasyPaisaNewFlow = async (
  ...
  try {
    ...
    if (channel == "DIRECT") {
      ...
      const token = await payfast.getApiToken(merchantId, decryptedPayload);
      if (!token?.token) {
      ...
      const validation = await payfast.validateCustomerInformation(merchantId, {
        token: token?.token,
        bankCode: '32',
        ...decryptedPayload
      })
      ...
      result = await payfast.pay(merchantId, {
        token: token?.token,
        bankCode: '32',
...
const initiateEasyPaisa = async (
  ...
  try {
    ...
    if (channel == "DIRECT") {
      ...
      const token = await payfast.getApiToken(req.params.merchantId as string, req.body);
      if (!token?.token) {
        console.log(JSON.stringify({ event: "PAYFAST_PAYIN_NO_TOKEN_RECIEVED", order_id: req.body.order_id }))
        throw new CustomError("No Token Recieved", 500);
      }
      const validation = await payfast.validateCustomerInformation(req.params.merchantId as string, {
        token: token?.token,
        bankCode: '32',
        ...req.body
      })
      ...
      result = await payfast.pay(req.params.merchantId as string, {
        token: token?.token,
        bankCode: '32',
        transaction_id: validation?.transaction_id,
        ...req.body
...
const initiateEasyPaisaAsync = async (
  ...
  try {
    ...
    if (channel == "DIRECT") {
      ...
      result = await payfast.payAsync(req.params.merchantId as string, {
        token: token?.token,
        bankCode: '32',
        transaction_id: validation?.transaction_id,
        ...req.body
...
const initiateEasyPaisaAsyncClone = async (
  ...
  try {
    ...
    if (channel == "DIRECT") {
      ...
      result = await payfast.payAsyncClone(req.params.merchantId as string, {
        token: token?.token,
        bankCode: '32',
        transaction_id: validation?.transaction_id,
        ...req.body
...
const createDisbursementCloneNewFlow = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    // 🔐 decrypted payload (from middleware)
    const decryptedPayload = req.body?.decryptedPayload;
    const merchantId = req.params?.merchantId as string;
    ...
    if (!merchantId) {
      res
        .status(400)
        .json(ApiResponse.error("Merchant ID is required"));
      return;
    }

    const payload: DisbursementPayload = decryptedPayload;
    ...
    const result = await easyPaisaService.createDisbursementClone(
      payload,
      merchantId
    );
...
const disburseThroughBankClone = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await easyPaisaService.disburseThroughBankClone(payload, merchantId as string);
    res.status(200).json(ApiResponse.success(result));
  }
  catch (err) {
    next(err);
...
export default {
  ...
  createDisbursementCloneNewFlow,
