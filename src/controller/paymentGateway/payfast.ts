...
const pay = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const payment = await payfast.pay(req.params.merchantId as string, {
            token: token?.token,
            bankCode: '32',
...
const  upaisaValidation = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const validation = await payfast.validateCustomerInformationForCnic(req.params.merchantId as string, {
            token: token?.token,
            bankCode: '14',
            ...req.body
        })
...
const upaisaPay = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const payment = await payfast.payCnic(req.params.merchantId as string, {
            token: token?.token,
            bankCode: '14',
            ...req.body
        })
...
const statusInquiry = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const method = (await payfast.getPayfastInquiryMethod(merchantId as string))?.payfastInquiryMethod;
        let result;
        if (method == "DATABASE") {
            result = await payfast.databaseStatusInquiry(merchantId as string, transactionId as string)
        }
        else {
            const token = await payfast.getApiToken(req.params.merchantId as string, req.body);
            if (!token?.token) {
            ...
            result = await payfast.payfastStatusInquiry(merchantId as string, transactionId as string, token?.token)
        }
        res.status(200).json(ApiResponse.success(result));
    }
    catch (err) {
