...
const login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    console.log(merchant[0].merchant?.billerId as number)

    let biller;
    if (merchant[0].merchant?.billerId) {
      biller = await prisma.merchantSubBillerCode.findUnique({
        where: {
          id: merchant[0].merchant?.billerId as number
        }
      })
    }
    ...
    console.log({
      ...
      sub_biller_code: biller?.biller_code
    })
    // Generate JWT token
