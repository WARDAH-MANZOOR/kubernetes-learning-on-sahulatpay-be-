...
const handleDialogpayIPN = async (req: Request, res: Response): Promise<void> => {
    try {
        const requestBody = req.body;
        const responseData = await ipnService.processDialogpayIPN(requestBody);
        res.json(responseData);
    }
    catch (error: any) {
        res.status(500).json(ApiResponse.error(error.message, 500));
...
const handleShopifyIPN = async (req: Request, res: Response): Promise<void> => {
    try {
        const requestBody = req.body;
        const responseData = await ipnService.processShopifyIPN(requestBody);
        res.json(responseData);
    }
    catch (error: any) {
        res.status(500).json(ApiResponse.error(error.message, 500));
...
export default { handleIPN, handleCardIPN, handlebdtIPN, handleShurjoPayIPN, handleDialogpayIPN, handleShopifyIPN };
