...
const getAllDisburseAccounts = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const result = await teleService.getAllDisburseAccounts();
        res.status(200).json(ApiResponse.success(result));
    }
    catch(err: any) {
        next(err);
...
const getAllDisburseAccountsWithMerchant = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const result = await teleService.getAllDisburseAccountWithAMerchant();
        res.status(200).json(ApiResponse.success(result));
    }
    catch(err: any) {
        next(err);
...
export default {
    ...
    getAllDisburseAccountsWithMerchant
}
