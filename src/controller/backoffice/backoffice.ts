...
const updateDisbursements = async (req: Request, res: Response) => {
    try {
...
const getUsdtRatesByPercentage = async (req: Request, res: Response) => {
    try {
        const result = await backofficeService.getUsdtRatesByPercentage();
        res.status(200).json(ApiResponse.success(result));
    } catch (err: any) {
        res.status(err.statusCode || 500).send(ApiResponse.error(err.message, err.statusCode || 500));
    }
