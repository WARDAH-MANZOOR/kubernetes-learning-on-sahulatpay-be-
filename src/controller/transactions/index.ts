  ...
  try {
    ...
    const response = {
      transactions: transactions.map((transaction) => ({
        ...
        merchant: {
          groups: transaction.merchant.groups.map((group) => ({
            merchant: { uid: group.merchant?.uid || null },
          })),
        },
...
const getProAndBal = async (req: Request, res: Response) => {
  try {
    ...
    // );
    const merchantsBalanceProfit = merchantId
  ? await getAllProfitsBalancesByMerchant(
      parseInt(merchantId as string), // merchantId first
      fromDate,                       // from date second
      toDate                          // to date third
    )
  : await getProfitAndBalance(fromDate, toDate); // no change
