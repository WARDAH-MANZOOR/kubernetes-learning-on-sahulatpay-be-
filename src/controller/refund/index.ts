...
const refundMWDisbursement = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const initTransaction = await refundService.refundMwTransaction(token?.access_token, req.body, req.params.merchantId as string);
...
const refundDisbursmentClone = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const token = await getToken(req.params.merchantId as string);
        const initTransaction = await refundService.refundIBFTTransaction(token?.access_token, req.body, req.params.merchantId as string);
        res.status(200).json(ApiResponse.success(initTransaction));
    }
    catch (err) {
        next(err)
