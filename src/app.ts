...
import failOldQrTransactions from './utils/qr_pending_fail_cron.js';
// // dotenv.config();
...
import {task, taskController} from "./utils/queue_task.js"
...
import { failedEasypaisaInquiry } from 'utils/jazzcash_failed_inquiry.js';
import fourTimesSettlementCron from 'utils/four_times_cron.js';
import bodyParser from "body-parser";

var app = express();


// cron.schedule("30 11 * * 1-5", task);
// cron.schedule("*/5 * * * *", pendingDisburse);
// cron.schedule("0 * * * *", cleanupCron.cleanupFailedAttempts)
// cron.schedule("* * * * *", pendingDisburse);
// cron.schedule('5 0,12 * * *', instantSettlementCron);
// cron.schedule('* * * * *', instantSettlementCron);
// cron.schedule("*/5 * * * *", expireOldReservations)
// cron.schedule("57 18 * * *", failedEasypaisaInquiry)
// cron.schedule("0 */4 * * *", failOldQrTransactions);
// cron.schedule('5 6,18 * * *', fourTimesSettlementCron)
