...
const verifyDerivedKeys = (
  masterKey: Buffer,
  expectedHmacKey: Buffer,
  expectedAesKey: Buffer
): boolean => {
  const { hmacKey, aesKey } = deriveKeys(masterKey);
  console.log({
    derivedHmacLen: hmacKey.length,
    expectedHmacLen: expectedHmacKey.length,
    derivedAesLen: aesKey.length,
    expectedAesLen: expectedAesKey.length,
  });
  return (
    timingSafeEqual(hmacKey, expectedHmacKey) &&
    timingSafeEqual(aesKey, expectedAesKey)
  );
};

const encryptAESGCM = (plaintext: string, aesKey: Buffer) => {
...
const decryptAESGCM = (encryptedData: string, aesKey: Buffer, iv: string, tag: string) => {
...
const generateHMACSignature = (data: string, hmacKey: Buffer) => {
