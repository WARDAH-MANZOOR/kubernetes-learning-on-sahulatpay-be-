...
import fs from "fs"
import { signNew } from "../../utils/signature.js";
import type {
  TransactionRequest,
  CompleteRequest,
...
const sendCallback = async (webhook_url: string, payload: any, msisdn: string, type: string, doEncryption: boolean, checkLimit: boolean, merchant: any) => {
  ...
  setTimeout(async () => {
    try {
      ...
      if (merchant?.additionalCallbackParams && type == "payin") {
        data = {...data, provider: (payload?.providerDetails as JsonObject)?.name as string}
      }
      ...
      if (type == "payin") {
        ...
        txn = await prisma.disbursement.findUnique({
          where: {
...
const moontonPrivateKey = fs.readFileSync("./src/keys/moonton_private.pem", "utf8");

const sendCallbackMoonton = async (
  webhook_url: string,
  payload: any,
  msisdn: string,
  type: string,
  ...
  setTimeout(async () => {
    try {
      // Updated payload according to documentation
      let data: any = {
        amount: (+payload.amount*100).toString(), // use correct minor unit
        merchant_order_id: `pay:order:${payload.merchant_order_id}` || payload.merchant_transaction_id,
        invoice_id: payload.invoice_id || payload.merchant_transaction_id,
        currency: payload.currency || "PKR",
        merchant_id: payload.merchant_id,
        status: payload.status,
        success_time: payload.date_time, // ISO string if needed
      };

      // Keep optional encryption if required internally
      // if (doEncryption) {
      //   data = await callbackEncrypt(JSON.stringify(data), payload?.merchant_id);
      // } else {
        data = JSON.stringify(data);
      // }

      console.log(`Data (${payload.merchant_transaction_id}): ${data}`);
      const signature = signNew(data, moontonPrivateKey);

      console.log("Signature to send in Authorization header:", signature);
      ...
      if (res.data == "success") {
        ...
        if (type == "payin") {
          await prisma.transaction.update({
            where: { merchant_transaction_id: payload.merchant_transaction_id },
            data: { callback_sent: true, callback_response: res.data }
          });
        } else {
          await prisma.disbursement.update({
            where: { merchant_custom_order_id: payload.merchant_transaction_id },
            data: { callback_sent: true, callback_response: res.data }
          });
        }
      } else {
        ...
        if (type == "payin") {
          await prisma.transaction.update({
            where: { merchant_transaction_id: payload.merchant_transaction_id },
            data: { callback_sent: false, callback_response: res?.data }
          });
        } else {
          await prisma.disbursement.update({
            where: { merchant_custom_order_id: payload.merchant_transaction_id },
