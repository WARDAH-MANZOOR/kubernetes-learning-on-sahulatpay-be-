...
export interface DialogpayIPNBody {
    order_id: string;
    status: string;
    amount: number;
    currency: string;
...
export interface ShopifyIPNBody {
...
const normalizeShopifyStatus = (status?: string): "completed" | "failed" | "pending" => {
    const normalized = String(status || "").trim().toLowerCase();
    if (["completed", "complete", "success", "successful", "paid", "000", "0000"].includes(normalized)) {
        return "completed";
    }
    if (["failed", "fail", "error", "declined", "canceled", "cancelled", "199", "999"].includes(normalized)) {
        return "failed";
    }
    if (["pending", "processing", "inprocess", "in-progress"].includes(normalized)) {
        return "pending";
    }
    return "pending";
};
    ...
    try {
        ...
        try {
            ...
            const forwardRes = await axios.post(
                "https://easypaisa-setup-server.assanpay.com/api/jazzcash/transactions/ipn",
                {
...
const processDialogpayIPN = async (requestBody: DialogpayIPNBody): Promise<any> => {
    try {
        console.log(JSON.stringify({ event: "DIALOGPAY_IPN_RECIEVED", order_id: requestBody?.order_id, requestBody }));
        if (!requestBody?.order_id) {
            throw new CustomError("Missing order_id", 400);
        }

        const txn = await prisma.transaction.findFirst({
            where: {
                OR: [
                    { merchant_transaction_id: requestBody.order_id },
        ...
        const finalStatus = normalizeShopifyStatus(requestBody.status);
        ...
        await prisma.transaction.updateMany({
            ...
            data: {
                status: finalStatus,
                response_message: requestBody.response_message || requestBody.status || "IPN received",
                providerDetails: {
                    ...(txn.providerDetails as JsonObject),
                    name: PROVIDERS.CARD,
                }
        ...
        await prisma.paymentRequest.updateMany({
            where: {OR: [{merchant_transaction_id: txn.merchant_transaction_id},{transactionId: txn.merchant_transaction_id}] },
            data: {
                status: "completed"
            }
        })
        if (finalStatus === "completed" && txn?.status !== "completed") {
            const findMerchant = await prisma.merchant.findUnique({
                where: {
                    merchant_id: txn?.merchant_id
                },
            ...
            if (txn && findMerchant?.webhook_url) {
                setTimeout(async () => {
                    transactionService.sendCallback(
                        findMerchant?.webhook_url as string,
                        txn,
...
const processShopifyIPN = async (requestBody: ShopifyIPNBody): Promise<any> => {
    try {
        console.log(JSON.stringify({ event: "EASYPAISA_IPN_RECIEVED", transaction_id: requestBody?.transaction_id, requestBody }));

        if (!requestBody?.transaction_id) {
            throw new CustomError("Missing transaction_id", 400);
        }

        const txn = await prisma.transaction.findFirst({
            where: {
                OR: [
                    { merchant_transaction_id: requestBody.transaction_id },
        ...
        const finalStatus = normalizeShopifyStatus(requestBody.status);
        const parsedAmount = requestBody.amount === undefined || requestBody.amount === null || requestBody.amount === ""
            ? undefined
            : Number(requestBody.amount);
        const parsedDate = requestBody.date_time ? new Date(requestBody.date_time) : null;
        ...
        await prisma.transaction.updateMany({
            ...
            data: {
                status: finalStatus,
                response_message: requestBody.status || "Payment completed",
                providerDetails: {
                    ...(txn.providerDetails as JsonObject),
                    payer_name: requestBody.name || null,
                    transactionId: requestBody.payment_session_id,
                }
            }
        });
        ...
        await prisma.paymentRequest.updateMany({
            where: {
                OR: [
                    { merchant_transaction_id: txn.merchant_transaction_id || requestBody.transaction_id },
                    { transactionId: txn.transaction_id }
                ]
            },
            data: {
...
export default { processIPN, processCardIPN, bdtIPN, processRaastQRIPN, processDialogpayIPN, processShopifyIPN };
