...
async function initiateTransaction(body: any, merchantId: string, token?: string) {
  let findMerchant: any;
  let walletBalance;
  let totalDisbursed: number | Decimal = new Decimal(0);
  ...
  try {
    ...
    //   , findDisbureMerch.key, findDisbureMerch.initialVector)
    const payload = {
      "orderId":body.order_id || id,"amount":+body.amount,"phone":body.phone,"iban":body.iban,"bankCode":body.bankCode,"merchantId":findMerchant?.merchant_id
    }
    let db_id = id;
    ...
    let response = await fetch(`${process.env.JAZZCASH_PAYOUT_URL}/ibft`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
    ...
    if (data.status != "completed") {
      ...
      await prisma.disbursement.create({
        data: {
          ...
          providerDetails: {
            name: res.accountName || res.categoryName
          }
        },
      });
      balanceDeducted = false;
      throw new CustomError(data.message, 500)
    }
    return await prisma.$transaction(
      async (tx) => {
        // Update transactions to adjust balances
...
async function mwTransaction(body: any, merchantId: string, token?: string) {
  let findMerchant: any;
  let walletBalance;
  let totalDisbursed: number | Decimal = new Decimal(0);
  let balanceDeducted = false;
  ...
  try {
    ...
    //     referenceId: id
    //   }, findDisbureMerch.key, findDisbureMerch.initialVector)
    const payload = { orderId: body.order_id || id, amount: +body.amount, phone: body.phone, cnic: body.cnic || "0000000000000", merchantId: JSON.stringify(findMerchant.merchant_id) }
    // const requestData = {
    //   data: payload
    // };
    ...
    const response = await fetch(`${process.env.JAZZCASH_PAYOUT_URL}/mw`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        // 'Authorization': `Bearer ${token}`,
...
async function mwTransactionClone(token: string, body: any, merchantId: string) {
  ...
  try {
    ...
    //     referenceId: id
    //   }, findDisbureMerch.key, findDisbureMerch.initialVector)
    const payload = { orderId: body.order_id, amount: +body.amount, phone: body.phone, iban: body.iban, bankCode: body.bankCode, merchantId: JSON.stringify(findMerchant.merchant_id) }
    const requestData = payload;
    const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));
...
async function checkTransactionStatus(token: string, body: any, merchantId: string) {
  ...
  for (const id of body.transactionIds) {
    ...
    try {
      ...
      results.push({ id, status: {merchantAmount: transaction.merchantAmount.toString(),...jsonResponse} });
    } catch (error: any) {
