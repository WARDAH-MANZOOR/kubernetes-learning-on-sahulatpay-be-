...
const initiateJazzCashPayment = async (
  ...
  try {
    ...
    if (paymentType === "CARD") {
      ...
      const paymentUrl =
        `${process.env.JAZZCASH_PAYIN_URL}?merchantId=${merchant?.merchant_id}`;
        ...
        let transaction = await tx.transaction.update({
          ...
          data: {
            ...
            providerDetails: {
              ...
              merchant: r?.accountName,
              sub_merchant: r?.mainCategoryName,
            },
      ...
      if (r.pp_ResponseCode === "000") {
        console.log(JSON.stringify({
          event: "JASSCASH_PAYIN_RESPONSE", orderId: paymentData.order_id, response: {
...
const initiateJazzCashPaymentForRedirection = async (
  paymentData: any,
  merchant_uid?: string
) => {
  let refNo: string = "";
...
const initiateJazzCashPaymentClone = async (
  paymentData: any,
  merchant_uid?: string
) => {
  let refNo: string = "";
...
const initiateSandboxJazzCashPayment = async (
  paymentData: any,
  merchant_uid?: string
) => {
  let refNo: string = "";
...
const initiateProductionJazzCashPayment = async (
  paymentData: any,
  merchant_uid?: string
) => {
...
const initiateJazzCashPaymentAsyncClone = async (
  paymentData: any,
  merchant_uid?: string
) => {
  console.log(JSON.stringify({ event: "JAZZCASH_ASYNC_INITIATED", order_id: paymentData.order_id, body: paymentData }))
    ...
    setImmediate(async () => {
      try {
        ...
        if (paymentData.type.toUpperCase() === "CARD") {
          ...
          await processWalletPaymentClone(sendData, refNo as string, result?.merchant?.merchant, phone, date, txnRefNo, result?.merchant?.jazzCashMerchant);
        } else {
          throw new CustomError("Invalid payment type", 400);
        }
      } catch (error: any) {
...
const initiateJazzCashCnicPayment = async (
  ...
  try {
    ...
    console.log(JSON.stringify({ event: "JAZZCASH_CNIC_INITIATED", order_id: paymentData.order_id, body: paymentData }))
    ...
    console.log(data);
    console.log(JSON.stringify({
      event: "JAZZCASH_CNIC_RESPONSE", order_id: paymentData.order_id, response: {
