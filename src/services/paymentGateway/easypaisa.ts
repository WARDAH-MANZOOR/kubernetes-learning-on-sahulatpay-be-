...
import bankCodeMapping from "../../data/bankCode_mapping.json" with { type: "json" };
// / 2️⃣ Define the type and assert it
type BankCodeMap = {
  [key: string]: string; // key = bank code, value = bank name
};
const bankCodeMappingTyped = bankCodeMapping as BankCodeMap;
import type {
  DisbursementPayload,
  IEasyLoginPayload,
...
const getExistingLimitReservationIds = async (merchantId: number, merchantTxnId?: string) => {
  if (!merchantTxnId) return [];

  const existingReservations = await prisma.limitReservation.findMany({
    where: {
      merchant_id: merchantId,
      provider: ProviderEnum.EASYPAISA,
      merchant_txn_id: merchantTxnId,
      status: {
        in: ["PENDING", "COMMITTED"],
      },
    },
    select: {
      id: true,
  ...
  return existingReservations.map((reservation) => reservation.id);
}
...
const initiateEasyPaisa = async (merchantId: string, params: any) => {
  ...
  try {
    ...
    const existingReservationIds = await getExistingLimitReservationIds(findMerchant.merchant_id, id2);
    if (existingReservationIds.length) {
      reservations = existingReservationIds;
    } else {
      const r = await reserveLimits({ merchantId: findMerchant.merchant_id, provider: ProviderEnum.EASYPAISA, amount: params.amount, merchantTxnId: id2 });
...
const corporateLogin = async (obj: IDisbursement) => {
  try {
    return await axios
      .post(
        "https://rgw.8798-f464fa20.eu-de.ri1.apiconnect.appdomain.cloud/tmfb/gateway/CorporateLogin/",
        {
...
const createDisbursementClone = async (
  ...
  try {
    ...
    // Phone number validation (must start with 92)
    if (!obj.phone.startsWith("92")) {
      obj.phone = "92" + obj.phone.slice(1);
    }
    ...
    const ma2ma: any = await axios
      .post(
        `${process.env.EASYPAISA_PAYOUT_URL}/mw`,
        { "orderId": data.merchant_custom_order_id, "amount": obj.amount, "phone": obj.phone, "merchantId": JSON.stringify(findMerchant.merchant_id) },
        {validateStatus: () => true}
      )
      .then((res) => res?.data)
      .catch((error) => {
    ...
    console.log("Response: ",ma2ma)
    data["transaction_id"] = ma2ma.transactionId;
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const txn = await prisma.disbursement.create({
          data: {
            ...
            provider: PROVIDERS.EASYPAISA,
            status: "failed",
            response_message: ma2ma.message,
            providerDetails: {
              id: findMerchant?.EasypaisaDisburseAccountId,
              name: ma2ma.accountName
            }
          },
        });
        console.log("Disbursement: ", txn)
        // return;
        throw new CustomError(ma2ma.message, 500);
      }, {
        // isolationLevel: Prisma.TransactionIsolationLevel.Serializable,
        isolationLevel: Prisma.TransactionIsolationLevel.RepeatableRead,
        maxWait: 60000,
    ...
    console.log("MW Transaction Error", error);
...
const disburseThroughBankClone = async (obj: any, merchantId: string) => {
  ...
  try {
    ...
    // If obj.bankCode exists, map it to BankName using bnkcode.json
    if (obj.bankCode) {
      const mappedBankName = bankCodeMappingTyped[obj.bankCode];
      if (!mappedBankName) {
        throw new CustomError("Invalid bank code", 400);
      }
      obj.bankName = mappedBankName; // override/add bankName
    }
    ...
    let data = {
      "orderId": data2["merchant_custom_order_id"],
      "order_id": data2["merchant_custom_order_id"],
      "amount": obj.amount,
      "phone": obj.phone,
    ...
    let config = {
      method: "post",
      maxBodyLength: Infinity,
      url: `${process.env.EASYPAISA_PAYOUT_URL}/ibft`,
      headers: headers,
      data: data,
      validateStatus: () => true
    };
    ...
    if (!res.data.success) {
      ...
      data2["transaction_id"] = res.data.transactionId || id;
      // Get the current date
      const date = new Date();
    ...
    return await prisma.$transaction(
      async (tx) => {
        data2["transaction_id"] = res.data.transactionId;
        // Get the current date
        const date = new Date();
