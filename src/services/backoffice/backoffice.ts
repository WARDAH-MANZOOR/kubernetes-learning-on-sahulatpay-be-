...
async function settleDisbursements(transactionIds: string[]) {
    try {
        ...
        for (const txn of txns) {
            ...
            let webhook_url;
            if (findMerchant?.payout_callback) {
...
async function settleAllMerchantTransactions(merchantId: number) {
    try {
        // Fetch merchant to check allowBalanceTransfer field
        ...
        }) as { allowBalanceTransfer: boolean } | null;
        ...
        if (merchantTxns.length <= 0) {
            throw new CustomError('No Transactions Found to settle', 404);
        }
        ...
        // Only transfer balance if allowBalanceTransfer is enabled for the merchant
        if (merchant.allowBalanceTransfer) {
            // Transfer merchantAmount from availableBalance to disbursementBalance
            // Deduct totalProviderDeduction and merchantAmount from wallet balance
            await backofficeService.adjustMerchantWalletBalance(
                merchantId,
                new Decimal(walletBalance).minus(totalProviderDeduction).minus(merchantAmount).toNumber(),
                false
            );

            // Add merchantAmount (after OTP deduction) to disbursementBalance
            const merchantAmountToTransfer = merchantAmount.minus(totalProviderDeduction);
            await backofficeService.adjustMerchantDisbursementBalance(
                merchantId,
                merchantAmountToTransfer.toNumber(),
                false,
                "in"
            );
        } else {
            // If balance transfer is not allowed, only deduct totalProviderDeduction from wallet balance
            await backofficeService.adjustMerchantWalletBalance(
                merchantId,
...
/**
 * Get USDT rates for fixed percentage buckets (0, 1, 2).
 * Returns "no rate found" when no record exists for a bucket.
 */
async function getUsdtRatesByPercentage() {
    try {
        const records = await prisma.merchantFinancialTerms.findMany({
            select: {
                usdtPercentage: true,
                usdtRate: true,
                updatedAt: true,
            ...
            distinct: ["usdtPercentage"],
        });

        const result = records.reduce((acc, record) => {
            const percentage = Number(record.usdtPercentage);
            acc[percentage] = {
                usdtPercentage: percentage,
                usdtRate: Number(record.usdtRate),
            };
            return acc;
        }, {} as Record<number, { usdtPercentage: number; usdtRate: number }>);

        return result;
    } catch (error: any) {
        throw new CustomError(error?.message || "Failed to fetch USDT rates", error?.statusCode || 500);
    }
}
...
async function createUSDTSettlementNew(body: any) {
    try {
        ...
        if (!chargebackAmount || chargebackAmount <= 0 || !walletAddress) {
            throw new CustomError("Invalid chargeback amount or wallet address", 400);
        }
...
const updateTransactions = async () => {
    try {
        ...
        const transactionIdsToFail = pendingTransactions
            .filter((txn) => {
                const providerName = String(
                    (txn.providerDetails as JsonObject | null)?.name ?? ''
                );
                return providerName !== PROVIDERS.QR;
            })
            .map((txn) => txn.transaction_id);

        if (transactionIdsToFail.length === 0) {
        ...
        const { count } = await prisma.transaction.updateMany({
            where: {
                transaction_id: { in: transactionIdsToFail },
            },
            data: {
                status: 'failed',
                response_message: 'System error',
