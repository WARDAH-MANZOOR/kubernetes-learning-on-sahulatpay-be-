...
const updateMerchant = async (payload: Merchant) => {
  ...
  try {
    ...
    let result = await prisma.$transaction(async (tx) => {
      ...
      const updateWhiteListData: any = {};

      if (company_name) {
        updateWhiteListData.company_name = company_name;
      }

      if (username) {
        updateWhiteListData.merchant_name = username; // ✅ Update merchant_name if username updated
      }

      if (payload.ips) {
        updateWhiteListData.ips = payload.ips; // ✅ Full list
      } else if (payload.ip) {
        updateWhiteListData.ips = [payload.ip]; // ✅ Single IP as list
      }

      if (Object.keys(updateWhiteListData).length > 0) {
        const existingWhiteListMerchant = await tx.whiteListMerchant.findUnique({
          where: { merchantId: +merchantId },
          select: { merchantId: true }
        });

        if (existingWhiteListMerchant) {
          await tx.whiteListMerchant.update({
            where: { merchantId: +merchantId },
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const merchant = await tx.merchant.create({
       ...
       // 👇 Add this block here
      const token = generateToken(user.id, company_name);
      await tx.whiteListMerchant.create({
        data: {
          merchantId: user.id,
          company_name: company_name,
          merchant_name:username,
          token,
          // ...(payload.ip && { ip: payload.ip }) // optional IP
          ...(payload.ip && { ips: [payload.ip] }), // ✅ Save single IP as a list
          ...(payload.ips && { ips: payload.ips })   // ✅ If already a list, use it
        }
      });
