...
const createMoontonPaymentRequest = async (data: any, user: any) => {
  try {
    if (!user) {
      throw new CustomError("User Id not given", 404);
    }
    ...
    const updatedPaymentRequest = await prisma.$transaction(async (tx) => {
      return tx.paymentRequest.update({
        ...
        data: {
          link: `/pay-moon/${newPaymentRequest.id}`,
        },
      });
    });
    ...
    return {
      message: "Payment request created successfully",
      data: { ...updatedPaymentRequest, completeLink: `https://merchant.sahulatpay.com/pay-moon/${newPaymentRequest.id}`, storeName: data.storeName, order_id: data.order_id, return_url: (newPaymentRequest.metadata as JsonObject)?.return_url },
    };
  } catch (error: any) {
    throw new CustomError(
      error?.message || "An error occurred while creating the payment request",
...
const createPaymentRequestForQR = async (data: any, user: any) => {
  try {
    ...
    if (findMerchant?.qrEnabled != true) {
      throw new CustomError("QR Not Enabled", 400)
    }
    ...
    const newPaymentRequest = await prisma.$transaction(async (tx) => {
      ...
      const paymentRequest = await tx.paymentRequest.create({
        data: {
          ...
          provider: data.provider,
          link: data.link,
          metadata: data.metadata || { phone: data.phone, return_url: data.link, failure_url: data.failure_url },
          createdAt: new Date(),
          updatedAt: new Date(),
          merchant_transaction_id: data.order_id,
        },
    ...
    return {
      message: "Payment request created successfully",
      data: { ...updatedPaymentRequest, completeLink: `https://merchant.sahulatpay.com/aik-qr/${newPaymentRequest.id}`, storeName: data.storeName, order_id: data.order_id, return_url: (newPaymentRequest.metadata as JsonObject)?.return_url, failure_url: data.failure_url },
    };
  } catch (error: any) {
    throw new CustomError(
      error?.message || "An error occurred while creating the payment request",
...
const payRequestedPayment = async (paymentRequestObj: any) => {
  try {
    ...
    const txn = await prisma.transaction.findUnique({
      where: {
        merchant_transaction_id: paymentRequest.merchant_transaction_id as string || paymentRequest.transactionId as string
      }
    })
    ...
    if (paymentRequestObj.provider?.toLocaleLowerCase() === "jazzcash") {
      ...
      let clientSecret = await zindigiService.generateNewClientSecret(merchant.uid);

      // Step 2: Attempt to use the client secret with the target API
      let validation = await zindigiService.walletToWalletPayment({ amount: paymentRequest.amount, mobile: transactionService.convertPhoneNumber(paymentRequestObj.accountNo) || transactionService.convertPhoneNumber((paymentRequest?.metadata as JsonObject)?.phone as string) }, merchant.uid, clientSecret);
    ...
    if (paymentRequestObj?.provider.toLowerCase() == "jazzcash" || paymentRequestObj?.provider.toLowerCase() == "easypaisa") {
      ...
      updatedPaymentRequest = await prisma.$transaction(async (tx) => {
        ...
        const merchantOrderId =
          ...
          paymentRequestObj.transaction_id ||
          paymentRequest.transactionId;

        const newPaymentRequest = merchantOrderId
          ? await tx.paymentOrderNew.findUnique({
              where: {
                merchantOrderId,
              },
            })
        ...
        if (newPaymentRequest && merchantOrderId) {
          moontonOrder = newPaymentRequest;
          await tx.paymentOrderNew.update({
            where: {
              merchantOrderId,
            },
            data: {
      ...
      if (moontonOrder) {
        console.log("Sending callback to moonton", {
          amount: moontonOrder.amount,
          merchant_order_id: moontonOrder.merchantOrderId,
          merchant_transaction_id: moontonOrder.merchantOrderId,
          invoice_id: moontonOrder.invoiceId,
          currency: moontonOrder.currency,
          merchant_id: moontonOrder.merchantId,
          status: "paid",
          success_time: new Date().toISOString(),
          date_time: new Date().toISOString(),
        })
        await transactionService.sendCallbackMoonton(
          moontonOrder.notifyUrl,
          {
            amount: moontonOrder.amount,
            merchant_order_id: moontonOrder.merchantOrderId,
            merchant_transaction_id: moontonOrder.merchantOrderId,
    ...
    try {
      ...
      const orderId = pr?.merchant_transaction_id || (pr?.transactionId as string);
      if (orderId) {
        const moontonOrder = await prisma.paymentOrderNew.findUnique({
        ...
        if (moontonOrder) {
          ...
          let encrypted = false;
          if (pr?.userId) {
            const merchantForEncryption = await prisma.merchant.findFirst({
              where: { merchant_id: pr.userId },
              select: { encrypted: true }
            });
            encrypted = merchantForEncryption?.encrypted === "True";
          }
          await transactionService.sendCallbackMoonton(
            moontonOrder.notifyUrl,
            {
...
const getPaymentRequestbyId = async (paymentRequestId: string) => {
  try {
    ...
    const newPaymentRequest = await prisma.paymentOrderNew.findUnique({
      where: {
        merchantOrderId: paymentRequest?.merchant_transaction_id || paymentRequest?.transactionId as string
      }
    })
    ...
    return {
      ...
      data: {
        ...paymentRequest,
        credentials: creds,
        return_url: (paymentRequest.metadata as JsonObject)?.return_url || newPaymentRequest?.returnUrl,
        merchant_of: credentials?.easyPaisaMerchant?.merchant_of,
        cardMethod: credentials?.cardMethod,
        failure_url: (paymentRequest.metadata as JsonObject)?.failure_url
      },
    };
  } catch (error: any) {
    throw new CustomError(
...
const getPaymentRequestbyTxnId = async (transactionId: string) => {
  try {
    ...
    if (!(paymentRequest?.merchant_transaction_id == transactionId || paymentRequest?.transactionId == transactionId)) {
      throw new CustomError("Payment request not found", 404);
    }
